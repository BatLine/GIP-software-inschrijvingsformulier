...
internal sealed class Helpers
{
    ...
    {
        Rectangle result;
        if (Subtract) { result = checked(new Rectangle(0, 0, S.Width - 1, S.Height - 1)); }
        else { result = new Rectangle(0, 0, S.Width, S.Height); }
        return result;
    }
...
public class XylosTabControl : TabControl
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        base.OnPaint(e);
        this.G.Clear(Helpers.ColorFromHex("#33373B")); //tabc tabcontrol main kleur
        checked
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                if (flag)
                {
                    ...
                    if (flag2)
                    {
                        using (SolidBrush solidBrush = new SolidBrush(Helpers.ColorFromHex("#2B2F33"))) //tabc selecteer kleur
                        {
                            using (SolidBrush solidBrush2 = new SolidBrush(Helpers.ColorFromHex("#BECCD9"))) //tabc text kleur
                            {
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        checked
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                if (flag) { this.OverIndex = i; break; }
                this.OverIndex = -1;
            }
        }
    }
